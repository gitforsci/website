...
Each block follows the same rhythm: motivate it, demonstrate it once ("my turn"), hand it over as a "your turn" task, then regroup. Some steps the whole room does together ("our turn"). The live drawings (@sec-live-drawings) are cued where they fit, and concept maps bookend the day (@sec-concept-maps).
...
- Read the room's own pre-course survey back to them, and name the focus learner you teach to (@sec-philosophy).
...
**Live drawing:** Drawing 1, local and remote (@sec-drawing-local-remote), as the summary after the hands-on step.
...
**Live drawing:** Drawing 2, the commit cycle (@sec-drawing-commit-cycle), as the summary.
...
**Live drawing:** Drawing 3, branching (@sec-drawing-branching), with the merge point left empty for Part 6.
...
**My turn:** open a pull request from `dev`, request a reviewer, leave line comments as the reviewer, then merge; back in RStudio, switch to `main` and pull. Say the habit out loud: merged, then pull `main` and switch back to `dev`.
...
| Part 4: Team work (push wall) | 20 min | Pair up, clone the partner's repo, try to push, hit the 403. The collaboration cliffhanger. |
...
| Part 6: Pull request, review, merge | 40 min | Open a PR, review with line comments, merge, pull `main`; author and reviewer roles, then swap. Drawing 4. Protected. |
