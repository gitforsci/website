...
![Drawing 2: the commit cycle. Leave room on the right of the history line; it grows as the day goes on.](images/drawing-2-commit-cycle.png){#fig-drawing-commit-cycle fig-alt="Two rounded boxes labelled working files (your edits) and staging area (what goes in), joined by a blue arrow labelled stage. A green arrow labelled commit points down from the staging area onto a horizontal history line carrying three labelled snapshot boxes c1, c2, and c3. An orange arrow labelled push leads from the history line to a box labelled GitHub (remote). A caption reads: each commit, a labeled snapshot you can walk back to."}

The editable source lives next to the image (`images/drawing-2-commit-cycle.excalidraw`). When drawing this by hand, plan the board so the history line has room to grow to the right.
...
The editable source lives next to the image (`images/drawing-3-branching.excalidraw`). Rehearse this one back-to-back with Drawing 4 at least once; they share the board and the story.
